...
Returning feedback requires an unresolved Review Comment or a Review Summary; either may accompany the other. Agent adapters consume the structured Feedback Bundle; manual reviewers use explicit JSON or Markdown copy actions. Approval with unresolved comments is allowed after a warning because reviewer authority, not comment state, determines the outcome.
